...
test("M8 operation contract、错误码和移动端交互", async ({ page }, testInfo) => {
  ...
  await expect(liveStatus).toContainText("CPU 使用率");
  await expect(liveStatus).toContainText("内存使用");
  await expect(liveStatus).toContainText("磁盘读取");
  await expect(liveStatus).toContainText("磁盘写入");
