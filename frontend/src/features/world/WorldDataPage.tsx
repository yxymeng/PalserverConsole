...
function CommunityCard({ resource, item, onOpenPal }: { resource: "guilds" | "bases"; item: WorldGuildListItem | WorldBaseListItem; onOpenPal: (id: string, trigger: HTMLButtonElement) => void }) {
  ...
  return <article className="world-community-card" data-kind={resource}>
    <header><div><h3>{item.name}</h3>{base && <p>{association}</p>}</div><span title={base ? "存档中的打工帕鲁数量 / 当前可用槽位；容量由游戏按据点等级与配置决定" : undefined}>{guild ? `${guild.memberCount.toLocaleString()} 成员 · ${guild.baseCount.toLocaleString()} 据点` : `${base?.workerCount.toLocaleString()} / ${base?.maxWorkerCount?.toLocaleString() ?? "—"} 打工帕鲁`}</span></header>
