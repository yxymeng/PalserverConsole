...
describe("玩家主要进度呈现", () => {
  ...
  it("只显示可验证且不小于玩家值的游戏资源总量", () => {
    const progress: WorldPlayerProgress = {
      state: "partial",
      values: { fastTravel: 48, exploredAreas: 87, towerBosses: 14 },
      unavailable: [],
    ...
    expect(playerProgressTotal(progress, "exploredAreas")).toBe(100);
    expect(playerProgressPercent(progress, "exploredAreas")).toBe(87);
    expect(playerProgressPercent(progress, "towerBosses")).toBeNull();
