import { AlertTriangle, ArchiveRestore, DatabaseBackup, FolderSearch, HardDrive, Infinity as InfinityIcon, Save, ShieldAlert, ShieldCheck } from "lucide-react";
...
import { formatBytes, formatObservedAt } from "../../utils/format";
...
export function BackupsPage({ auth }: { auth: AuthStatus }) {
  ...
  const validBackups = data?.items.filter((item) => item.valid).length || 0;
  const totalBytes = data?.items.reduce((total, item) => total + item.sizeBytes, 0) || 0;
  return <section className="maintenance-section maintenance-backups" id="maintenance-backups" aria-labelledby="maintenance-backups-title">
    ...
    <section className="backup-heading"><div><span className="backup-heading-icon"><DatabaseBackup aria-hidden="true" /></span><div><h2 id="maintenance-backups-title">官方备份</h2><p>管理 PalServer 官方 backup/world 历史快照；活动世界始终没有删除入口。</p></div></div>{auth.local && <button className="quiet-button" onClick={() => void openDirectory()}><FolderSearch size={17} />打开目录</button>}</section>
    ...
    <section className="backup-overview" aria-label="备份概览">
      <article><ArchiveRestore aria-hidden="true" /><span>历史快照</span><strong>{data?.items.length || 0}</strong><small>{validBackups} 份可安全恢复</small></article>
      <article><HardDrive aria-hidden="true" /><span>磁盘占用</span><strong>{formatBytes(totalBytes)}</strong><small>仅统计官方备份目录</small></article>
      <article>{retention === "unlimited" ? <InfinityIcon aria-hidden="true" /> : <ShieldCheck aria-hidden="true" />}<span>自动清理</span><strong>{retention === "unlimited" ? "不启用" : `保留 ${retention} 份`}</strong><small>只清理最旧完整备份</small></article>
    </section>
    <section className="backup-policy"><div><h3>保留策略</h3><p>策略独立保存在控制台数据库中，不修改 PalServer 配置文件。</p></div><form className="backup-retention-form" onSubmit={saveRetention}><label htmlFor="backup-retention">保留数量</label><Select value={retention} onValueChange={(value) => setRetention(value || "unlimited")}><SelectTrigger id="backup-retention" aria-label="保留数量"><SelectValue>{retention === "unlimited" ? "不限（不自动清理）" : `保留最近 ${retention} 份`}</SelectValue></SelectTrigger><SelectContent><SelectGroup>{[{ value: "unlimited", label: "不限（不自动清理）" }, { value: "3", label: "保留最近 3 份" }, { value: "5", label: "保留最近 5 份" }, { value: "10", label: "保留最近 10 份" }, { value: "20", label: "保留最近 20 份" }, { value: "30", label: "保留最近 30 份" }, { value: "50", label: "保留最近 50 份" }].map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectGroup></SelectContent></Select><button className="primary-button" type="submit"><Save size={17} />保存</button></form></section>
    ...
    <section className="backup-ledger" aria-label="官方备份列表"><header><div><h3>备份记录</h3><p>按目录时间查看大小、完整性和恢复入口。</p></div><span>{data?.items.length || 0} 份</span></header>{data?.items.length ? <div className="backup-ledger-list">{data.items.map((item, index) => <BlurFade key={item.id} delay={Math.min(index, 5) * 0.035} duration={0.26} offset={5}><article className="backup-ledger-item" data-valid={item.valid || undefined}><span className="backup-ledger-status" aria-hidden="true">{item.valid ? <ShieldCheck /> : <ShieldAlert />}</span><div className="backup-ledger-main"><div><strong>{item.id}</strong><span>{formatObservedAt(item.observedAt)}</span></div><p>{item.valid ? "结构完整，可用于恢复" : `结构不完整：缺少 ${item.missing.join(", ")}`}</p><details><summary>查看存储位置</summary><code>{data.backupRoot}\{item.id}</code></details></div><strong className="backup-ledger-size">{formatBytes(item.sizeBytes)}</strong><div className="backup-ledger-actions"><button className="quiet-button" disabled={!item.valid} onClick={() => setPendingAction({ kind: "restore", id: item.id })}>恢复</button><button className="danger-button" onClick={() => setPendingAction({ kind: "delete", id: item.id })}>删除</button></div></article></BlurFade>)}</div> : <p className="empty-state">暂无官方备份。</p>}</section>
