...
4. 脚本会先校验新包 `checksums.sha256`，读取新包支持的数据库 schema 上限，并在 `data\upgrade-backups\<时间戳>\` 复制 `app.db`。之后只替换 `Program/`，绝不替换或删除 `data/`。

如果现有 `app.db` 的 schema 比候选版本更新，脚本会以 `INCOMPATIBLE_DOWNGRADE` 拒绝降级，旧 `Program/` 保持不变。升级的文件校验或替换失败时，脚本会自动恢复旧 `Program/`，并保留数据库备份。成功升级后，旧程序保留在根目录 `program-backups/Program-<时间戳>`，可在停止控制台后人工回退；人工回退旧程序时不要回退或覆盖 `data/`。
