...
如果 `8223` 已被其他程序占用，未配置 `AdminPassword` 的控制台不会用 `SO_REUSEADDR` 与其他服务共享回环端口，而是明确报错。已配置密码并允许可信 LAN 时，如果冲突只发生在某个具体 IPv4 接口，控制台会保留 `127.0.0.1` 和其他未占用接口；浏览器仍打开本机 IPv4 地址，不再回退到同端口 IPv6。
