...
帕鲁列表有昵称时只显示昵称，无昵称时显示中文种族名；性别以名字旁的 `♂` / `♀` 小图标表示，属性列只显示闪光、头目、狂暴、觉醒、导入角色和浓缩等级。帕鲁详情使用唯一的“当前位置”说明它在随身队伍、帕鲁终端还是据点工作，并附带对应训练家或据点名称；归属缺失时显示“归属资料不可用”，不回退显示内部 ID，也不会同时重复显示所属训练家与存放位置。

据点默认显示全部公会的结果，可通过公会下拉菜单搜索并选择单个公会。缺少公会关联的据点仍保留在全部结果中，并显示“公会关联资料不可用”；它不是“未关联公会”或“自由据点”类别。明确标注为 `Base ID`、`Guild ID` 等技术字段仍保留用于诊断。头像及中文目录为离线资源，来源、固定版本和许可证见 `THIRD_PARTY_NOTICES.md`；可捕获人类/NPC 若上游没有专属头像，会显示明确的未知头像。
