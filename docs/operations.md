...
帕鲁列表有昵称时只显示昵称，无昵称时显示中文种族名；性别以名字旁的 `♂` / `♀` 小图标表示，属性列只显示闪光、头目、狂暴、觉醒、导入角色和浓缩等级。主人、据点和工会等关系列显示对应名称；关联资料缺失时显示明确的“资料不可用”，不回退显示内部 ID。明确标注为 `Base ID`、`Guild ID` 等技术字段仍保留用于诊断。头像及中文目录为离线资源，来源、固定版本和许可证见 `THIRD_PARTY_NOTICES.md`；可捕获人类/NPC 若上游没有专属头像，会显示明确的未知头像。
