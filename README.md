...
首版已经覆盖本地控制台的核心闭环，但仍有明确限制：真实服务器的恢复、配置应用和破坏性操作需要维护窗口；存档解析依赖本机已有的兼容环境；当前没有自动更新器或独立安装包。

后续版本计划按优先级推进：

1. 提供更简单的便携版/安装包，减少首次配置步骤。
2. 增强 Palworld 新版本存档解析兼容性，并继续保持只读校验。
3. 增加备份计划、导出和更细的恢复前检查。
4. 扩展只读监控指标、玩家活动和审计筛选。
5. 提供升级迁移检查，确保升级控制台时不覆盖 `data/` 中的用户运行数据。

这些是后续方向，不代表首版已经提供对应功能。
